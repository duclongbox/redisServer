...
func cmdSADD(args []string) []byte{
	if len(args) < 2{
		return Encode(errors.New("(errors) ERR wrong number of arguments for 'SADD' command"),false)
...
func cmdSREM(args []string) []byte{
	if len(args) < 2{
		return Encode(errors.New("(errors) ERR wrong number of arguments for 'SREM' command"),false)
...
func cmdSMEMBERS(args []string) []byte{
	if len(args) != 1{
		return Encode(errors.New("(errors) ERR wrong number of arguments for 'SMEMBERS' command"),false)
...
func cmdSISMEMBER(args []string) []byte{
	if len(args) != 2{
		return Encode(errors.New("(errors) ERR wrong number of arguments for 'SISMEMBER' command"),false)
	}
	key := args[0]
	set,exist := setStore[key]
	if !exist{
